...
class brs(object):
    ...
    def propose(self, rules_curr, rules_norm, q):
        ...
        if len(incorr) == 0:
            clean = True
            move = ['clean']
            # it means the BRS correctly classified all points but there could be redundant patterns, so cleaning is needed
        else:
            clean = False
        ...
        if len(move) > 0 and move[0] == 'add':
            ...
            if random() < q:
                ...
                mat = np.multiply(self.RMatrix[Yhat_neg_index, :].transpose(), np.array(self.Y)[Yhat_neg_index])
