...
def make_results(dataset, whichtype, num_runs, costs, validation=False, asym_costs=[1,1]):
    ...
    for run in bar(range(num_runs)):
        ...
        for cost in costs:
            ...
            tr_mod = load_results(dataset, whichtype, run, cost, 'tr2stage')
            ...
            for i in range(50):
                ...
                if validation: 
                    ...
                    hyrs_model_preds, hyrs_model_covered, _ = hyrs_mod.predict(x_test, human_decisions) #hyrs_mod.predict(x_test, human_decisions, with_reset=True, conf_human=human_conf, p_y=e_y_mod.predict_proba(x_test_non_binarized))
                    hyrs_mod_confs = hyrs_mod.get_model_conf_agreement(x_test, human_decisions)[0] #.get_model_conf_agreement(x_test, human_decisions, prs_min=hyrs_mod.prs_min, nrs_min=hyrs_mod.nrs_min)[0]
                    
                    hyrs_team_preds = hyrs_mod.humanifyPreds(hyrs_model_preds, human_decisions, human_conf, human.ADB, x_test) #.predictHumanInLoop(x_test, human_decisions, human_conf, human.ADB, with_reset=True, p_y=e_y_mod.predict_proba(x_test_non_binarized))[0]
                    ...
                    hyrs_norecon_model_preds = hyrs_norecon_mod.predict(x_test, human_decisions)[0]
                    hyrs_norecon_team_preds = hyrs_norecon_mod.humanifyPreds(hyrs_norecon_model_preds, human_decisions, human_conf, human.ADB, x_test)#.predictHumanInLoop(x_test, human_decisions, human_conf, human.ADB, with_reset=True, p_y=e_y_mod.predict_proba(x_test_non_binarized))[0] #
                    brs_team_preds = brs_humanifyPreds(brs_model_preds, brs_conf, human_decisions, human_conf, human.ADB)
